...
def _mm1_qkt(
    ...
    DMA + MMA sequence for each KV block.  Single workspace slot per core
    (no ring index).
    """
    # wait workspace_s slot free (Vec1 released after consuming scores)
...
def _mm2_pv(
    ...
    MMA and writes the partial output to workspace_pv for Vec2.  Serial
    version: single workspace slot per core (no ring index).
    """
...
def _vec1_softmax(
    ...
    neg_max_bp = tl.make_block_ptr(workspace_neg_max + cid * BLOCK_M, (BLOCK_M, 1), (1, 1), (0, 0), (BLOCK_M, 1),
                                   (1, 0))
    if idx_in_conbine == 0:
...
def _vec2_accumulate(
    ...
    """
    # wait workspace_pv (all CB P*V blocks) ready from MM2
...
#  tile.copy; cross-engine ordering uses sync_block_set / sync_block_wait.
#  The four steps per task (MM1 → Vec1 → MM2 → Vec2) execute sequentially
#  within each loop iteration — no pipeline overlap, no ring buffer.
# =============================================================================
@triton.jit
def flash_attention_fwd_serial_kernel(
    ...
    #  The four sub-functions are merged into two scopes; cross-engine
    ...
    for g in range(num_global_tasks):
        ...
        #  MM1 signals SEM_S_READY; MM2 waits SEM_P_READY (set by Vec1).
        # -----------------------------------------------------------------
        with tle.scope(core_mode="cube"):
            _mm1_qkt(
                Q,
        ...
        # -----------------------------------------------------------------
        #  Vector scope — Vec1: softmax(workspace_s) -> workspace_p
        #                 Vec2: accumulate pv into acc_o; write output on last block
        #  Vec1 waits SEM_S_READY (set by MM1) and signals SEM_P_READY;
...
def _dump_signature():
    ...
    ptr = {
        ...
        "workspace_pv": "*fp16", "workspace_rescale": "*fp32", "workspace_expsum": "*fp32", "workspace_neg_max": "*fp32"
    }
    i32_names = [
