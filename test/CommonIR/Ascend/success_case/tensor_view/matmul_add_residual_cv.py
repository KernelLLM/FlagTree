...
def _cube_matmul(
    ...
    # store matmul result into per-core GM workspace slot: workspace is laid
    # out as NUM_CORES stacked (BLOCK_M, BLOCK_N) slots along dim 0, so the
    # per-core offset is the dim-0 tile index `cid`, not raw pointer
    # arithmetic on `workspace` itself.
    ws_view = tl.make_tensor_view(workspace, (NUM_CORES * BLOCK_M, BLOCK_N), (BLOCK_N, 1))
    tl.store(ws_view, acc.to(workspace.dtype.element_ty), index=(cid, 0), tile=(BLOCK_M, BLOCK_N))
...
def _vector_add_residual(
    ...
    pid_m = m_start // BLOCK_M
    pid_n = n_start // BLOCK_N

    # load matmul result from GM workspace (see _cube_matmul: workspace is
    # NUM_CORES stacked (BLOCK_M, BLOCK_N) slots along dim 0)
    ws_view = tl.make_tensor_view(workspace, (NUM_CORES * BLOCK_M, BLOCK_N), (BLOCK_N, 1))
    mm_tile = tl.load(ws_view, index=(cid, 0), tile=(BLOCK_M, BLOCK_N)).to(tl.float32)

    # load residual tile
    residual_view = tl.make_tensor_view(residual, (M, N), (N, 1))
    res_tile = tl.load(residual_view, index=(pid_m, pid_n), tile=(BLOCK_M, BLOCK_N)).to(tl.float32)

    # element-wise add and store to output
    out_tile = mm_tile + res_tile
    mat_c_view = tl.make_tensor_view(mat_c, (M, N), (N, 1))
    tl.store(mat_c_view, out_tile.to(mat_c.dtype.element_ty), index=(pid_m, pid_n), tile=(BLOCK_M, BLOCK_N))
