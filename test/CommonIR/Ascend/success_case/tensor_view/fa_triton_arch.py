...
def _mm1_qkt(
    ...
    #if idx_in_conbine == 0:
    q_view = tl.make_tensor_view(Q, (B, Hq, S, DIM), (sQb, sQh, sQs, sQd))
    q_block4 = tl.load(q_view, index=(batch_idx, head_idx, global_head_idx, 0), tile=(1, 1, BLOCK_M, DIM))
    q_l1 = tl.reshape(q_block4, (BLOCK_M, DIM))

    for cb_idx in range(CB):
        kv_idx = idx_in_conbine * CB + cb_idx

        # Load K as a plain register tensor then transpose for Q @ K^T
        k_view = tl.make_tensor_view(K, (B, Hkv, S, DIM), (sKb, sKh, sKs, sKd))
        k_block4 = tl.load(k_view, index=(batch_idx, kv_head_idx, kv_idx, 0), tile=(1, 1, BLOCK_N, DIM))
        k_block = tl.reshape(k_block4, (BLOCK_N, DIM))
        ...
        # workspace_s is [NUM_CORES, RING, CB, BLOCK_M, BLOCK_N] stacked along
        # dim 0 as (NUM_CORES*RING*CB*BLOCK_M) rows; combined_idx is the
        # dim-0 tile index (in units of BLOCK_M rows).
        combined_idx = cid * RING * CB + ring_slot * CB + cb_idx
        score_view = tl.make_tensor_view(workspace_s, (NUM_CORES * RING * CB * BLOCK_M, BLOCK_N), (BLOCK_N, 1))
        tl.store(score_view, attn_score_l0c.to(workspace_s.dtype.element_ty), index=(combined_idx, 0),
...
def _mm2_pv(
    ...
    for cb_idx in range(CB):
        ...
        combined_idx = cid * RING * CB + prev_ring_slot * CB + cb_idx

        # Load V from GM as a plain register tensor (matches fa_4func.py pattern)
        v_view = tl.make_tensor_view(V, (B, Hkv, S, DIM), (sKb, sKh, sKs, sKd))
        v_block4 = tl.load(v_view, index=(prev_batch_idx, kv_prev_head_idx, prev_kv_idx, 0), tile=(1, 1, BLOCK_N, DIM))
        v_block = tl.reshape(v_block4, (BLOCK_N, DIM))

        # Load P from workspace_p as a plain register tensor (see _mm1_qkt:
        # [NUM_CORES, RING, CB, BLOCK_M, BLOCK_N] stacked along dim 0)
        prob_view = tl.make_tensor_view(workspace_p, (NUM_CORES * RING * CB * BLOCK_M, BLOCK_N), (BLOCK_N, 1))
        p_block = tl.load(prob_view, index=(combined_idx, 0), tile=(BLOCK_M, BLOCK_N))

        # Both operands are plain tensors: no tile_to_tensor, no 4D cbuf issue.
        pv_part_l0c = tl.dot(p_block, v_block, tl.zeros((BLOCK_M, DIM), tl.float32))

        pv_view = tl.make_tensor_view(workspace_pv, (NUM_CORES * RING * CB * BLOCK_M, DIM), (DIM, 1))
        tl.store(pv_view, pv_part_l0c.to(workspace_pv.dtype.element_ty), index=(combined_idx, 0), tile=(BLOCK_M, DIM))
...
def _vec1_softmax(
    ...
    for cb_idx in range(CB):
        ...
        # load score[0:BLOCK_M, :] from workspace_s GM -> UB (see _mm1_qkt:
        # [NUM_CORES, RING, CB, BLOCK_M, BLOCK_N] stacked along dim 0)
        score_view = tl.make_tensor_view(workspace_s, (NUM_CORES * RING * CB * BLOCK_M, BLOCK_N), (BLOCK_N, 1))
        attn_score_block = tl.load(score_view, index=(combined_idx, 0), tile=(BLOCK_M, BLOCK_N)).to(tl.float32)
        ...
        rescale_view = tl.make_tensor_view(workspace_rescale, (NUM_CORES * RING * CB * BLOCK_M, 1), (1, 1))
        tl.store(rescale_view, rescale, index=(combined_idx, 0), tile=(BLOCK_M, 1))
        expsum_view = tl.make_tensor_view(workspace_expsum, (NUM_CORES * RING * CB * BLOCK_M, 1), (1, 1))
        tl.store(expsum_view, block_expsum, index=(combined_idx, 0), tile=(BLOCK_M, 1))
        ...
        prob_view = tl.make_tensor_view(workspace_p, (NUM_CORES * RING * CB * BLOCK_M, BLOCK_N), (BLOCK_N, 1))
        tl.store(prob_view, softmax_p.to(workspace_p.dtype.element_ty), index=(combined_idx, 0),
                tile=(BLOCK_M, BLOCK_N))
...
def _vec2_accumulate(
    ...
    for cb_idx in range(CB):
        ...
        pv_view = tl.make_tensor_view(workspace_pv, (NUM_CORES * RING * CB * BLOCK_M, DIM), (DIM, 1))
        pv_acc = tl.load(pv_view, index=(combined_idx, 0), tile=(BLOCK_M, DIM)).to(tl.float32)

        # load rescale and block_expsum written by Vec1 for this slot
        rescale_view = tl.make_tensor_view(workspace_rescale, (NUM_CORES * RING * CB * BLOCK_M, 1), (1, 1))
        rescale = tl.load(rescale_view, index=(combined_idx, 0), tile=(BLOCK_M, 1)).to(tl.float32)
        expsum_view = tl.make_tensor_view(workspace_expsum, (NUM_CORES * RING * CB * BLOCK_M, 1), (1, 1))
        block_expsum = tl.load(expsum_view, index=(combined_idx, 0), tile=(BLOCK_M, 1)).to(tl.float32)
        ...
        if prev_kv_idx == NUM_KV_BLOCKS - 1:
            ...
            output_block4 = tl.reshape(output_block, (1, 1, BLOCK_M, DIM))
            o_view = tl.make_tensor_view(Out, (B, Hq, S, DIM), (sOb, sOh, sOs, sOd))
            tl.store(o_view, output_block4,
                    index=(prev_batch_idx, prev_head_idx, prev_global_head_idx.to(tl.int32), 0),
                    tile=(1, 1, BLOCK_M, DIM))
