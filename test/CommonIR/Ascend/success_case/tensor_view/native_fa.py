...
def _attn_fwd(Q, K, V, sm_scale, M, Out,  #
    ...
    tl.store(m_ptrs, m_i)
    o_view = tl.make_tensor_view(Out, (Z, H, N_CTX, HEAD_DIM), (stride_ob, stride_oh, stride_om, stride_od))
    acc_out = tl.expand_dims(tl.expand_dims(acc.to(tl.float16), 0), 0)
    tl.store(o_view, acc_out, index=(off_z, off_h, start_m, 0), tile=(1, 1, BLOCK_M, HEAD_DIM))
