...
def _mm1_qkt(
    ...
    # workspace_s is NUM_CORES stacked (BLOCK_M, BLOCK_N) slots along dim 0
    score_view = tl.make_tensor_view(workspace_s, (NUM_CORES * BLOCK_M, BLOCK_N), (BLOCK_N, 1))
    tl.store(score_view, attn_score.to(workspace_s.dtype.element_ty), index=(cid, 0), tile=(BLOCK_M, BLOCK_N))
...
def _mm2_pv(
    ...
    v_block4 = tl.load(v_view, index=(batch_idx, kv_head_idx, kv_idx, 0), tile=(1, 1, BLOCK_N, DIM))
    v_block = tl.reshape(v_block4, (BLOCK_N, DIM))

    # workspace_p is NUM_CORES stacked (BLOCK_M, BLOCK_N) slots along dim 0
    prob_view = tl.make_tensor_view(workspace_p, (NUM_CORES * BLOCK_M, BLOCK_N), (BLOCK_N, 1))
    p_block = tl.load(prob_view, index=(cid, 0), tile=(BLOCK_M, BLOCK_N))

    # pv_part = P * V  — (BLOCK_M, BLOCK_N) @ (BLOCK_N, DIM) = (BLOCK_M, DIM)
    pv_part = tl.dot(p_block, v_block, tl.zeros((BLOCK_M, DIM), tl.float32))

    # workspace_pv is NUM_CORES stacked (BLOCK_M, DIM) slots along dim 0
    pv_view = tl.make_tensor_view(workspace_pv, (NUM_CORES * BLOCK_M, DIM), (DIM, 1))
    tl.store(pv_view, pv_part.to(workspace_pv.dtype.element_ty), index=(cid, 0), tile=(BLOCK_M, DIM))
...
def _vec1_softmax(
    ...
    """
    # load score written by MM1 (workspace_s is NUM_CORES stacked (BLOCK_M,
    # BLOCK_N) slots along dim 0)
    score_view = tl.make_tensor_view(workspace_s, (NUM_CORES * BLOCK_M, BLOCK_N), (BLOCK_N, 1))
    attn_score_block = tl.load(score_view, index=(cid, 0), tile=(BLOCK_M, BLOCK_N)).to(tl.float32)
    ...
    # softmax_p -> workspace_p (read by MM2; NUM_CORES stacked slots along dim 0)
    prob_view = tl.make_tensor_view(workspace_p, (NUM_CORES * BLOCK_M, BLOCK_N), (BLOCK_N, 1))
    tl.store(prob_view, softmax_p.to(workspace_p.dtype.element_ty), index=(cid, 0), tile=(BLOCK_M, BLOCK_N))
...
def _vec2_accumulate(
    ...
    """
    # load pv_part written by MM2 (workspace_pv is NUM_CORES stacked
    # (BLOCK_M, DIM) slots along dim 0)
    pv_view = tl.make_tensor_view(workspace_pv, (NUM_CORES * BLOCK_M, DIM), (DIM, 1))
    pv_acc = tl.load(pv_view, index=(cid, 0), tile=(BLOCK_M, DIM)).to(tl.float32)
...
def flash_attention_fwd_3task_kernel(
    ...
    for tile_idx in range(block_num):
        ...
        q_block = tl.reshape(q_block4, (BLOCK_M, DIM))
        ...
        out_block4 = tl.reshape(out_block, (1, 1, BLOCK_M, DIM))
        o_view = tl.make_tensor_view(Out, (B, Hq, S, DIM), (sOb, sOh, sOs, sOd))
        tl.store(o_view, out_block4, index=(batch_idx, head_idx, global_head_idx.to(tl.int32), 0),
