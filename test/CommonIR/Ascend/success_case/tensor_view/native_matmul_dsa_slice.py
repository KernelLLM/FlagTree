...
def matmul_kernel(
    ...
    # tl.tensor_view over the whole logical A/B/C matrices; the K-loop below
    # indexes A/B by (pid_m or pid_n, k_idx) tile position instead of
    # advancing a block pointer, and the epilogue store indexes C by
    # (pid_m, pid_n).
    mat_a_view = tl.make_tensor_view(mat_a, (M, K), (K, 1))
    mat_b_view = tl.make_tensor_view(mat_b, (K, N), (N, 1))
    ...
    for block_idx in range(pid, NUM_BLOCKS, NUM_CORES):
        ...
        for k_idx in range(0, NUM_K_BLOCKS):
            ...
            a_tile_loaded = tl.load(mat_a_view, index=(pid_m, k_idx), tile=(BLOCK_M, BLOCK_K))
            a_l1_updated = tle.dsa.insert_slice(a_l1_full, a_tile_loaded, offsets=[a_row_off, 0],
            ...
            b_tile_loaded = tl.load(mat_b_view, index=(k_idx, pid_n), tile=(BLOCK_K, BLOCK_N))
            b_l1_updated = tle.dsa.insert_slice(b_l1_full, b_tile_loaded, offsets=[0, b_col_off],
