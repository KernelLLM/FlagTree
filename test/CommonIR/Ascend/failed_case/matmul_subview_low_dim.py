...
def matmul_kernel(
    ...
    mat_a_l1 = tile_alloc([BLOCK_M, NUM_SLOT * BLOCK_K], mat_a.dtype.element_ty, L1)
    mat_b_l1 = tile_alloc([BLOCK_K, NUM_SLOT * BLOCK_N], mat_b.dtype.element_ty, L1)
    ...
    for block_idx in range(pid, NUM_BLOCKS, NUM_CORES):
        ...
        # folds to a compile-time constant, giving tile_subview static offsets
        # that BiShengHIR can handle.  Compute-first ordering (dot before
        # tile_copy) prevents fold_staging_copy from hoisting all copies ahead
        # of all computes.
        NUM_MAIN: tl.constexpr = NUM_K_BLOCKS_CE - (NUM_SLOT - 1)
