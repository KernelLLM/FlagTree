...
def _tile_pipe_available(builder) -> bool:
    return (_tileir_mode() and hasattr(builder, "create_tile_pipe_writer_acquire")
            and hasattr(builder, "create_tile_pipe_writer_commit")
            and hasattr(builder, "create_tile_pipe_writer_close")
            and hasattr(builder, "create_tile_pipe_reader_wait")
            and hasattr(builder, "create_tile_pipe_reader_release"))
...
class buffered_tensor(tl.base_value):
    ...
    def slot(self, stage, _semantic=None):
        ...
        if _tileir_mode():
            ...
            slot_handle = _semantic.builder.create_tile_subview(
                self.handle,
                offsets,
                slot_shape,
                [1] * len(slot_shape),
                slot_ty.to_memdesc_ir(_semantic.builder),
            )
...
class buffered_tensor_type(tl.block_type):
    ...
    def to_memdesc_ir(self, builder: ir.builder) -> None:
        shape = self.shape
        builder = self.semantic.builder
...
class pipe_writer(_pipe_endpoint):
    ...
    def acquire(self, iter, _semantic=None):
        ...
        if _tile_pipe_available(_semantic.builder):
            _semantic.builder.create_tile_pipe_writer_acquire(
                self.pipe._field_handles(), stage.handle, phase.handle, self.pipe.capacity, self.pipe.scope,
    ...
    def commit(self, iter, _semantic=None):
        ...
        if _tile_pipe_available(_semantic.builder):
            _semantic.builder.create_tile_pipe_writer_commit(
                self.pipe._field_handles(), stage.handle, self.pipe.capacity, self.pipe.scope,
                self.pipe._ir_name(), self.pipe._field_names())
        else:
    ...
    def close(self, iter, _semantic=None):
        ...
        stage, phase = self.pipe._stage_phase(iter, _semantic=_semantic)
        if _tile_pipe_available(_semantic.builder):
            _semantic.builder.create_tile_pipe_writer_close(
                self.pipe._field_handles(), stage.handle, phase.handle, self.pipe.capacity, self.pipe.scope,
                self.pipe._ir_name(), self.pipe._field_names())
        else:
...
class pipe_reader(_pipe_endpoint):
    ...
    def wait(self, iter, _semantic=None):
        stage, phase = self.pipe._stage_phase(iter, _semantic=_semantic)
        if _tile_pipe_available(_semantic.builder):
            is_closed = _semantic.builder.create_tile_pipe_reader_wait(
                self.pipe._field_handles(), stage.handle, phase.handle, self.pipe.capacity, self.pipe.scope,
                self.pipe._ir_name(), self.pipe._field_names(), self.reader_name or "", self._reader_field_names())
        else:
            is_closed = _semantic.builder.create_pipe_reader_wait(
                self.pipe._field_handles(), stage.handle, phase.handle, self.pipe.capacity, self.pipe.scope,
                self.pipe._ir_name(), self.pipe._field_names(), self.reader_name or "", self._reader_field_names())
    ...
    def release(self, iter, _semantic=None):
        stage, _ = self.pipe._stage_phase(iter, _semantic=_semantic)
        if _tile_pipe_available(_semantic.builder):
            _semantic.builder.create_tile_pipe_reader_release(
                self.pipe._field_handles(), stage.handle, self.pipe.capacity, self.pipe.scope,
                self.pipe._ir_name(), self.pipe._field_names(), self.reader_name or "", self._reader_field_names())
        else:
