...
def _tileir_available(builder) -> bool:
    return hasattr(builder, "create_tile_alloc")
...
def _tile_storage_name(storage: tle.scope) -> str:
    ...
    if storage == tle.tmem:
        return "local"
    raise ValueError(f"Storage type {storage} not supported by TileIR")
...
def alloc(
    ...
    try:
        ...
        if _tileir_available(_semantic.builder):
            ...
            tensor_handle = _semantic.builder.create_tile_alloc(tile_ty, layout_handle)
            result = tle.buffered_tensor(tensor_handle, dtype, unwrapped_shape, storage, layout, _semantic)
            if init_value is not None:
                _semantic.builder.create_tile_store_tensor(init_value.handle, tensor_handle)
            return result
...
def copy(
    ...
    if not isinstance(shape, (tuple, list)):
        ...
        if hasattr(shape, '__iter__'):
            ...
            raise ValueError(f"Shape parameter must be tuple or list, but got {type(shape)}")
    if hasattr(_semantic.builder, "create_tile_copy"):
        tile_indices = []
        if not is_normcopy:
            if offsets is None:
                raise ValueError("TMA copy requires offsets")
            tile_indices = _semantic._convert_to_ir_values(offsets, require_i64=False)
        _semantic.builder.create_tile_copy(src.handle, dst.handle, tile_indices, False)
        return
    if is_normcopy:
...
def to_tensor(
    ...
    if hasattr(_semantic.builder, "create_tile_to_tensor"):
...
def store_tensor(tensor_value: tl.tensor, dst: tle.buffered_tensor, _semantic=None) -> None:
    ...
    if hasattr(_semantic.builder, "create_tile_store_tensor"):
...
def local_ptr(
    ...
    if hasattr(_semantic.builder, "create_tile_local_ptr"):
        local_ptr_op = _semantic.builder.create_tile_local_ptr(result_ir, buffer.handle, *handles)
    else:
